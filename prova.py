...
btn_download = tk.Button(root, text="Scarica", font=("Arial", 12), command=scarica_click_handler,
                         bg=BTN_COLOR, fg=FG_COLOR, activebackground="#444", activeforeground=FG_COLOR)
btn_download.pack(pady=10)

def delete_file_with_retry(file_path, retries=3, delay=0.5):
    for attempt in range(retries):
        try:
            ...
            if attempt < retries - 1:
                ...
                print(f"Errore: impossibile eliminare il file {file_path} - {e}")
                return False
        except Exception as e:
            print(f"Errore imprevisto durante l'eliminazione del file {file_path}: {e}")
            return False
...
def on_close():
    ...
    if messagebox.askyesno("Conferma", "Vuoi interrompere l'importazione dei file?"):
        safe_progress("Interruzione in corso...")
        dots_frame.pack_forget()
        logo_label.pack_forget()
        btn_download.pack_forget()
        ...
        try:
            os.makedirs(LOCAL_IMPORT_DIR, exist_ok=True)
            with open(SESSIONE_PATH, "w") as f:
                f.write("Importazione interrotta manualmente.")
        except Exception as e:
            print(f"Errore durante la creazione di sessione.txt: {e}")

        # Aspetta che il thread termini (max 50 sec)
        if import_thread and import_thread.is_alive():
            import_thread.join(timeout=50)
            if import_thread.is_alive():
                print("ATTENZIONE: Il thread di importazione non è terminato entro 50 secondi.")

        # Elimina i file
        files_to_delete = FILE_LIST + ["PROMOZIONI", "DOCFOR"]
        for file_name in files_to_delete:
            file_path = os.path.join(LOCAL_IMPORT_DIR, file_name)
